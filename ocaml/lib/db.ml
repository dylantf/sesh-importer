...
let connection_uri =
  Uri.of_string
    "postgresql://dylan:dylan@localhost:5432/seshtracker_dev"
;;
...
let insert_sesh_query =
  Caqti_request.Infix.(Caqti_type.(tup4 string string int (tup2 (option string) (option string))) ->! Caqti_type.int)
    "INSERT INTO seshes (user_id, date, sport, duration_seconds, location_name, comments) \
     VALUES (1, $1::date, $2, $3, $4, $5) RETURNING id"
;;

let insert_wind_details_query =
  Caqti_request.Infix.(Caqti_type.(tup4 int int int string) ->. Caqti_type.unit)
    "INSERT INTO wind_sesh_details (sesh_id, wind_avg, wind_gust, sesh_type) \
     VALUES ($1, $2, $3, $4)"
;;

let insert_gear_query =
  Caqti_request.Infix.(Caqti_type.(tup2 int int) ->. Caqti_type.unit)
...
let insert_wind_details (module Db : Caqti_lwt.CONNECTION) n sesh_id =
  ...
  | Some avg, Some gust, Some st ->
    Db.exec insert_wind_details_query (sesh_id, avg, gust, stringify_sesh_type st)
    |> Lwt_main.run
    |> or_fail
...
let insert_gear (module Db : Caqti_lwt.CONNECTION) n sesh_id =
  ...
  if ids <> [] then begin
    ...
    |> List.iter (fun gear_id ->
      Db.exec insert_gear_query (sesh_id, gear_id)
      |> Lwt_main.run
      |> or_fail)
  end
;;
...
let with_connection f =
  match Caqti_lwt.connect connection_uri |> Lwt_main.run with
  | Ok db ->
    let result = f db in
